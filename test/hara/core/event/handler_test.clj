(ns hara.core.event.handler-test
  (:use hara.test)
  (:require [hara.core.event.handler :refer :all]))

^{:refer hara.core.event.handler/new-id :added "3.0"}
(fact "creates a random id with a keyword base"
  (new-id)
  ;;=> :06679506-1f87-4be8-8cfb-c48f8579bc00
)

^{:refer hara.core.event.handler/expand-data :added "3.0"}
...
^{:refer hara.core.event.handler/check-data :added "3.0"}
...
^{:refer hara.core.event.handler/manager :added "3.0"}
...
^{:refer hara.core.event.handler/remove-handler :added "3.0"}
...
^{:refer hara.core.event.handler/add-handler :added "3.0"}
(fact "adds a handler to the manager"
  (-> (add-handler (manager) :hello {:id :hello
                                     :handler identity})
      (match-handlers {:hello "world"})
...
^{:refer hara.core.event.handler/list-handlers :added "3.0"}
(fact "list handlers that are present for a given manager"

  (list-handlers (manager))
  => [])

^{:refer hara.core.event.handler/match-handlers :added "3.0"}
